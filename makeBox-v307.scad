...
makeText=false;
showpins = false;
showSensorBoard=false;
showStripboardClearance=false;


// If we use wood screws, M2.5, the pilot hole is 1mm:
pcbMountScrewDiameter=2;
...
module makeEverything(x, y, z, padx=10, pady=10, wall=2, extx=0, exty=0, extendBase=0, extendSide=0, showlid=false, makeSides=true) {
    ...
    // fill the holes at front with thin layer of 1mm
    translate([9.3,139.1,-1.50]) color("red") {
        // close extra window
        translate([-19,-22,0]) cube([29.5,29,1]) ;
        // close bh1745
        translate([+12,-17,0]) cube([30,22,1]) ;
        // close bh1750
        translate([+36,0,0]) cube([30,24,1]) ;
        // close sht
        if (closesht_for_version_a) {
            translate([+5,0,0]) cube([30,24,1]) ;
        };
        translate([+36,0,0]) cube([30,24,1]) ;
        translate([+0,0,+0.0]) {
            coverAda2();
            if (makeText) translate([5.5,12.0,0]) text("sht45*6", size=5);
        };
        translate([+32.0,-2.7,+0.0]) {
            coverAda1();
            if (makeText) translate([5,12.0,0]) text("[bh1750]", size=5);
        };
        translate([-23.9,-45.6,+0.0]) {
            coverAda();
            if (makeText) translate([6.2,12.0,0]) text("AHT20*12", size=4);
        };
        translate([+4,-31,+0.0]) {
            coverpim();
            if (makeText) translate([6.2,22.0,0]) text("bh1745*0", size=4);
        };
        translate([+5,-8,+0.0]) {
            coverpim();
            if (makeText) translate([13.2,-23.0,0]) text("ltr*12", size=4);
        };
        translate([+35,-23.3,+0.0]) {
            cover3();
            if (makeText) translate([14,12.0,0]) text("i2s*12", size=4);
        };
        translate([+31,-38.0,+0.0]) {
            cover4();
            if (makeText) translate([13,0.0,0]) text("mcp*12", size=4);
    ...
    coversizey=3.5;
module cover4() {     
        translate([28,7.5,+0.0]) cube([coversizex,coversizey,1]) ;
        translate([13,7.5,+0.0]) cube([coversizex,coversizey,1]) ;

};    

module cover3() {     
        translate([25,7.5,+0.0]) cube([coversizex,coversizey,1]) ;
        translate([13,7.5,+0.0]) cube([coversizex,coversizey,1]) ;
...
module coverpim() {     
        translate([25,7.5,+0.0]) cube([coversizex,coversizey,1]) ;
        translate([11,7.5,+0.0]) cube([coversizex,coversizey,1]) ;

};    

module coverAda1() {     
    translate([7,20.3,+0.0]) cube([coversizex,coversizey,1]) ;
        translate([27,20.3,+0.0]) cube([coversizex,coversizey,1]) ;
        translate([27.2,7.5,+0.0]) cube([coversizex,coversizey,1]) ;
        translate([6.6,7.5,+0.0]) cube([coversizex,coversizey,1]) ;

};    

module coverAda2() {     // XXX
    translate([5.9,20.3,+0.0]) cube([coversizex,coversizey,1]) ;
        translate([27,20.3,+0.0]) cube([coversizex,coversizey,1]) ;
        translate([27.2,7.5,+0.0]) cube([coversizex,coversizey,1]) ;
        translate([6.6,7.5,+0.0]) cube([coversizex,coversizey,1]) ;
...
module coverAda() {     
    translate([7,20,+0.0]) cube([coversizex,coversizey,1]) ;
        translate([27,20,+0.0]) cube([coversizex,coversizey,1]) ;
        translate([27,7.5,+0.0]) cube([coversizex,coversizey,1]) ;
        translate([7,7.5,+0.0]) cube([coversizex,coversizey,1]) ;

};    
    translate([70,-7,1]) color("red") {
        rotate([0,0,90]) translate([5,60,0.9]) {
            // cube([0.4,0.4,0.4]);
            linear_extrude(0.4) text(
                closesht_for_version_a ? str("OpenDevEd ", thedate, "a") : str("OpenDevEd ",thedate, "b" ) 
            , size=6, font="Ubuntu:style=Bold");
        };
        label("^OpenDevEd");
        translate([-7,0,0]) label(thedate);
        //translate([-15,0,0]) label("2023 05 30");
        //translate([-25,0,0]) label("2023.05.30");
        //translate([-35,0,0]) label("2023/05/30");
    };
...
module baseplatemaker(x,y,z, padx=20, pady=20, wall=2, extx=0, exty=100, extendBase, showlid=false, mountpointheight=5, lid_is_laser_cut = false) {
    ...
    module baseplatemakerMain() {
        ...
        //RENDER
        translate([-padx,-pady,+2]) translate([108.4,-1.6,86.49]) rotate([90,180,0]) color([1,1,0,0.8])  import("2006-16step/camdenboss-lid.stl");
    ...
    module sensorSchablone(colonly=false, showpinsonly=false, schablone=true, schablone_diameter=1.4) {        
        ...
        translate([25.4/10,0,0]) {
            ...
            translate([+0*tinch,-9*tinch,0])        
                translate([-13.11,120.98,-wall]) {
                    ...
                    mount_ada_st_size1_pins4(1,1,col="green", mount_pins_above_sensor=true,  mount_pins_adjust=mpadjust, colonly=colonly, showpins=makeObjects, showpinsonly=showpinsonly,schablone=schablone, schablone_diameter=schablone_diameter);
                };                
            // unused area
            if (schablone) {
            ...
            translate([+9*tinch,+0*tinch,0]) 
                ...
                mount_ada_st_size1_pins5(col="orange", mount_pins_above_sensor=true,  mount_pins_adjust=mpadjust, colonly=colonly,  showpins=makeObjects, showpinsonly=showpinsonly,schablone=schablone, schablone_diameter=schablone_diameter);  
                // cutout for protrusion
                color("green") translate([+13.2,13,-1]) cube([7.5,2.5,3]);     
            };
            // col 2 - Pim
            ...
            translate([+16.76,118.44,-wall]) {
                ...
                mount_garden(1,1, col="lightgreen",mount_pins_above_sensor=true,  mount_pins_adjust=mpadjust, colonly=colonly, extendx=5, extendy=3, showpins=makeObjects, showpinsonly=showpinsonly,schablone=schablone,schablone_diameter=schablone_diameter); 
            };         
            translate([1*tinch,+1*tinch,0])     
            translate([+16.76,118.44,-wall]) {
                if (makeText) translate([0,7,0]) color("black") text("BH1745",size=2);
                mount_garden(1,1, col="lightgreen",mount_pins_above_sensor=true,  mount_pins_adjust=mpadjust, colonly=colonly,extendx=5, extendy=1.78,  showpins=makeObjects, showpinsonly=showpinsonly,schablone=schablone, schablone_diameter=schablone_diameter);            
            };
        };
        // col 3 - 
        translate([+2*tinch,-19*tinch,0]) 
            translate([+42.78,146.58,-wall]) {
                ...
                mount_ada_mcp(col="green",mount_pins_above_sensor=true,  mount_pins_adjust=mpadjust, extendx=7.5, colonly=colonly , showpins=makeObjects, showpinsonly=showpinsonly,schablone=schablone, schablone_diameter=schablone_diameter);
            };
        ...
        // extendx=10,  showpins=makeObjects, showpinsonly=showpinsonly,schablone=schablone); };
            translate([+26*tinch,14*tinch,+3]) 
                translate([-13.75,97.84,-wall]) { 
                    if (makeText) translate([0,7,0]) color("black") text("I2S",size=2);
                    rotate([180,0,0]) mount_ada_mems_i2s(schablone=schablone,schablone_diameter=schablone_diameter);
        };
        // col2-3, sht45
        translate([+22*tinch,-1*tinch,0]) translate([-13.11,143.84,-wall]) {
            if (makeText) translate([0,7,0]) color("black") text("BH1750",size=2);
            mount_ada_st_size1_pins6(col="orange", mount_pins_above_sensor=true,  mount_pins_adjust=mpadjust, colonly=colonly,  showpins=makeObjects, showpinsonly=showpinsonly,schablone=schablone, schablone_diameter=schablone_diameter);    
        };               
       };
    };
    ...
    module stripboard(nx, ny, npadx=2, npady=2,
        ...
        if (showcylinder) color("gree") getCylinderSet(height=50);
    ...
    module cutoutScreen() {
        ten=2;
        extend=1;
        cutoutx = 14.5-ten/2-extend;
        cutouty = +0.5-ten/2;
        cutoutdx = 38+ten+extend;
        cutoutdy = 86+ten;
    ...
    module pcbMountHole(x, y, zoffset, height,  lid_is_laser_cut) {
        ...
        // color("red") cylinder(d=1.0,h=100);
